...
**Exception:** Two screens use `DraggableFlatList` from `react-native-draggable-flatlist` instead of FlashList because they require drag-to-reorder functionality:
...
- `src/screens/playlist-detail.tsx` – conditionally renders `DraggableFlatList` when the user enters edit mode to reorder tracks.
